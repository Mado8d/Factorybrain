...
def check_node_health():
    ...
    with SyncSession() as session:
        ...
        for node in stale_nodes:
            ...
            logger.warning(f"Node {node.id} marked inactive — last seen {node.last_seen}")
        ...
        if total_stale or total_revived:
            logger.info(f"Health check: {total_stale} nodes marked inactive, {total_revived} nodes re-activated")

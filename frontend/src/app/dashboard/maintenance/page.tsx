...
export default function MaintenancePage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Onderhoud</h1>
        ...
        <div className="space-y-3">
          {alerts.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border">
              <p className="text-gray-500">Geen alerts</p>
            </div>
          ) : (
            alerts.map((alert) => (
              <div
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-gray-400">
                      {new Date(alert.created_at).toLocaleString('nl-BE')}
                    </span>
                  </div>
                </div>
                {alert.anomaly_score != null && (
                  <p className="text-sm text-gray-500 mt-2">
                    Anomalie score: {(alert.anomaly_score * 100).toFixed(0)}%
                  </p>
                )}
              </div>
            ))
      ...
      {activeTab === 'work-orders' && (
        <div className="space-y-3">
          {workOrders.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border">
              <p className="text-gray-500">Geen werkorders</p>
            </div>
          ) : (
            workOrders.map((wo) => (
                  ...
                  <div className="text-right">
                    ...
                    {wo.scheduled_date && (
                      <p className="text-xs text-gray-400 mt-1">
                        Gepland: {new Date(wo.scheduled_date).toLocaleDateString('nl-BE')}
                      </p>
                    )}
                  </div>
                </div>
